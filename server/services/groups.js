...
import {LARGE, MEDIUM, SMALL, SINGULAR, MEDIUM_MIN, SMALL_MIN, LARGE_MIN, LARGE_RADIUS, MEDIUM_RADIUS, SMALL_RADIUS, LARGE_ZOOM_LIMIT, SMALL_ZOOM_LIMIT, MEDIUM_ZOOM_LIMIT, SINGULAR_ZOOM_LIMIT} from "../utils/constants/group.constants.js";
import {METER_CONST} from "../utils/constants/calculation.constants.js";
import WorkerManager from "../workers/workermanager.js";
...
class GroupService{
	...
	async createGroupRecord(type, children=null){
		...
		const pos = (children != null ? await WorkerManager.exec("centerGroup", [children]) : {latitude: -1, longitude: -1});
		...
		try{
			...
			if(lineUp[0].groupType == SINGULAR){
				...
				if(firstLayerSize >= this.setGroupMinimum(SMALL)){
					...
					const ids = await WorkerManager.exec("mapGroupIds", [firstLayer]);
